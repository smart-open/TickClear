...
- 版本基线：**v2.14.0（versionCode 24，2026-08-11 封板）** — **首次启动权限引导全链路交付**：新建独立 DataStore + `PermissionChecker` 扩展 6 项检测（通知 / 精确闹钟 / 后台电池白名单 / 叠加显示 / 位置 / 麦克风 / 相机），与主设置存储完全解耦（不参与备份导入/导出，避免跨设备恢复时重置已完成的引导状态）；`PermissionsIntroScreen` 通用化为引导页与复访页共用同一 Composable（4 步分页、状态卡 + ChannelList + 一键直达系统设置的 Intent 助手），设置页「高级 → 助手配置 → 权限配置」新增复访入口；`MainActivity` 集成引导遮罩；splash→引导顺序回归修复（避免引导页遮住开机动画）+ 配置页视觉美化（4 圆点指示器、80dp 大图标容器、16dp 圆角 StatusCard、48dp 圆角底栏按钮）。导航仍为五大 Tab，工具箱仍为 **8 大类 53 个** 离线工具。历史版本详见 [docs/release-notes](docs/release-notes.md)。
- 成熟度：四维（产品设计 / 软件开发 / 质量测试 / 应用使用配置）均 **≥99**，综合 **99.9 / 100**，详见 [成熟度评估](docs/成熟度评估.md)。
