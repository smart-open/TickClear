 ...
 * 排程已下沉到注入的 [TaskScheduler]（接口），测试用普通类 mock（relaxed）隔离，
 * 避免 mockkObject 在「带默认参数 + companion EntryPoint」的 object 上行为不稳定。
 */
...
class AddTaskUseCaseTest {
    ...
    fun `插入任务并返回冲突列表`() = runTest {
        ...
        val useCase = AddTaskUseCase(scheduler, repo, ConflictChecker)
    ...
    fun `无冲突时返回空冲突`() = runTest {
        ...
        val result = AddTaskUseCase(scheduler, repo, ConflictChecker)(newTask)
    ...
    fun `开启提醒的任务落库后统一排程`() = runTest {
        ...
        AddTaskUseCase(scheduler, repo, ConflictChecker)(newTask)

        // 先撤旧闹钟，再排新闹钟（下沉兜底：任何路径新建任务都排程）
        coVerify(exactly = 1) { scheduler.cancelForTask(newTask.id) }
        coVerify(exactly = 1) { scheduler.scheduleForTask(newTask) }
    }
    ...
    fun `未开提醒的任务只撤旧闹钟不排程`() = runTest {
        ...
        coVerify(exactly = 1) { scheduler.cancelForTask(newTask.id) }
        coVerify(exactly = 0) { scheduler.scheduleForTask(any()) }
    }
}
